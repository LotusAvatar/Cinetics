...
public class KinectManager : MonoBehaviour 
{
    ...
    private float dynamicBoxTimer = 0f;
...
//								float yNorm = 1.0f - (float)posColor.y / sensorData.colorImageHeight;
    ...
    /// <summary>
	/// Gets and sets the hand interaction box dynamically for a specified user.
	/// </summary>
	/// <returns><c>true</c>, if the hand interaction box was gotten, <c>false</c> otherwise.</returns>
	/// <param name="userId">User ID</param>
	/// <param name="handJointType">Variable sat as right or left hand</param>
	/// <param name="leftBotBack">Vector containing the left, bottom and back coordinates, in meters</param>
	/// <param name="rightTopFront">Vector containing the right, top and front coordinates, in meters</param>
	/// <param name="leftBotBackDifference">Vector containing the left, bottom and back last relative distance from the initial hand point</param>
	/// <param name="rightTopFrontDifference">Vector containing the right, top and front last relative distance from the initial hand point</param>
	/// <param name="spineDistance">Vector containing the relative distance from the initial hand position and the middle spine position</param>
	/// <param name="bValidBox">If set to <c>true</c>, the previously set coordinates are valid</param>
	public bool GetHandDynamicInteractionBox(Int64 userId, KinectInterop.JointType handJointType,  ref Vector3 leftBotBack, ref Vector3 rightTopFront, ref Vector3 leftBotBackDifference, ref Vector3 rightTopFrontDifference, Vector3 spineDistance, bool bValidBox, ref bool dynamicBox)
    {
        if (dictUserIdToIndex.ContainsKey(userId))
        {
            int index = dictUserIdToIndex[userId];
                ...
                bodyFrame.bodyData[index].bIsTracked != 0)
            {
                ...
                if (bodyData.joint[(int)KinectInterop.JointType.SpineMid].trackingState != KinectInterop.TrackingState.NotTracked)
                {
                    Vector3 handPosition = bodyData.joint[(int)handJointType].position;
                    Vector3 spineReferencePos = bodyData.joint[(int)KinectInterop.JointType.SpineMid].position + spineDistance;
                    Vector3 currentDifference = handPosition - spineReferencePos;
                    bool checkDynamicBox = dynamicBox;

                    if (dynamicBox)
                    {
                        checkDynamicBox = false;

                        if (currentDifference.x >= 0f)
                        {
                            if (rightTopFrontDifference.x < currentDifference.x)
                            {
                                rightTopFrontDifference.x = currentDifference.x;
                                checkDynamicBox = true;
                            }
                        }
                        else
                        {
                            if (leftBotBackDifference.x > currentDifference.x)
                            {
                                leftBotBackDifference.x = currentDifference.x;
                                checkDynamicBox = true;
                            }
                        }

                        if (currentDifference.y >= 0f)
                        {
                            if (rightTopFrontDifference.y < currentDifference.y)
                            {
                                rightTopFrontDifference.y = currentDifference.y;
                                checkDynamicBox = true;
                            }
                        }
                        else
                        {
                            if (leftBotBackDifference.y > currentDifference.y)
                            {
                                leftBotBackDifference.y = currentDifference.y;
                                checkDynamicBox = true;
                            }
                        }

                        if (currentDifference.z <= 0f)
                        {
                            if (rightTopFrontDifference.z > currentDifference.z)
                            {
                                rightTopFrontDifference.z = currentDifference.z;
                                checkDynamicBox = true;
                            }
                        }
                        else
                        {
                            if (leftBotBackDifference.z < currentDifference.z)
                            {
                                leftBotBackDifference.z = currentDifference.z;
                                checkDynamicBox = true;
                            }
                        }
                        ...
                        if (!checkDynamicBox)
                        {
                            dynamicBoxTimer += Time.deltaTime;
                            if (dynamicBoxTimer > 5f)
                                dynamicBox = false;
                        }
                        else
                        {
                    ...
                    rightTopFront = spineReferencePos + rightTopFrontDifference;
                    leftBotBack = spineReferencePos + leftBotBackDifference;
                }
                else
                {
                    bResult = bValidBox;
	...
	public bool GetHandInteractionBox(Int64 userId, KinectInterop.JointType handJointType,  ref Vector3 leftBotBack, ref Vector3 rightTopFront, Vector3 leftBotBackDifference, Vector3 rightTopFrontDifference, Vector3 spineDistance, bool bValidBox)
	{
		if (dictUserIdToIndex.ContainsKey(userId))
		{
			int index = dictUserIdToIndex[userId];
