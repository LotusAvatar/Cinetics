...
/// </summary>
public class LauncherInteractionManager : MonoBehaviour
{
    /// <summary>
    ...
    private Vector3 spineMidPos = Vector3.zero;
    ...
    private Vector3 leftDynamicIboxLeftBotBack = Vector3.zero;
    private Vector3 leftDynamicIboxRightTopFront = Vector3.zero;
    private Vector3 leftHandSpineMidDistance= Vector3.zero;
    private bool firstLeftHandPositionInited = false;
    ...
    private Vector3 rightDynamicIboxLeftBotBack = Vector3.zero;
    private Vector3 rightDynamicIboxRightTopFront = Vector3.zero;
    private Vector3 rightHandSpineMidDistance = Vector3.zero;
    private bool firstRightHandPositionInited = false;
    ...
    private static LauncherInteractionManager instance;
    ...
    public static LauncherInteractionManager Instance
    {
        get
        {
            return instance;
    ...
    void Update()
    {
        ...
        if (kinectManager && kinectManager.IsInitialized())
        {
            ...
            if (primaryUserID != 0)
            {
                ...
                if (!firstLeftHandPositionInited)
                {
                    if (kinectManager.GetJointTrackingState(primaryUserID, (int)KinectInterop.JointType.HandLeft) != KinectInterop.TrackingState.NotTracked &&
                        kinectManager.GetJointTrackingState(primaryUserID, (int)KinectInterop.JointType.SpineMid) != KinectInterop.TrackingState.NotTracked)
                    {
                        firstLeftHandPositionInited = true;

                        leftHandPos = kinectManager.GetJointPosition(primaryUserID, (int)KinectInterop.JointType.HandLeft);
                        spineMidPos = kinectManager.GetJointPosition(primaryUserID, (int)KinectInterop.JointType.SpineMid);

                        leftHandSpineMidDistance = leftHandPos - spineMidPos;
                        leftDynamicIboxLeftBotBack = Vector3.zero;
                        leftDynamicIboxRightTopFront = Vector3.zero;
                    }
                    return;   
                }

                bool dynamicBox = true;
                isLeftIboxValid = kinectManager.GetHandDynamicInteractionBox(primaryUserID, KinectInterop.JointType.HandLeft, ref leftIboxLeftBotBack, ref leftIboxRightTopFront, ref leftDynamicIboxLeftBotBack, ref leftDynamicIboxRightTopFront, leftHandSpineMidDistance, isLeftIboxValid, ref dynamicBox);
                   ...
                   kinectManager.GetJointTrackingState(primaryUserID, (int)KinectInterop.JointType.HandLeft) != KinectInterop.TrackingState.NotTracked)
                {
                    ...
                    leftHandScreenPos.z = Mathf.Clamp01((leftHandPos.z - leftIboxLeftBotBack.z) / (leftIboxRightTopFront.z - leftIboxLeftBotBack.z));

                    bool wasLeftHandInteracting = isLeftHandInteracting;

                    if (kinectManager.GetJointTrackingState(primaryUserID, (int)KinectInterop.JointType.HandRight) == KinectInterop.TrackingState.NotTracked)
                    {
                    ...
                    else
                    {
                        ...
                        isLeftHandInteracting = (rightHandPos.z >= leftHandPos.z);
                    }
                    /*
                ...
                if (!firstRightHandPositionInited)
                {
                    if (kinectManager.GetJointTrackingState(primaryUserID, (int)KinectInterop.JointType.HandRight) != KinectInterop.TrackingState.NotTracked &&
                        kinectManager.GetJointTrackingState(primaryUserID, (int)KinectInterop.JointType.SpineMid) != KinectInterop.TrackingState.NotTracked)
                    {
                        firstRightHandPositionInited = true;

                        rightHandPos = kinectManager.GetJointPosition(primaryUserID, (int)KinectInterop.JointType.HandRight);
                        spineMidPos = kinectManager.GetJointPosition(primaryUserID, (int)KinectInterop.JointType.SpineMid);

                        rightHandSpineMidDistance = rightHandPos - spineMidPos;
                        rightDynamicIboxLeftBotBack = Vector3.zero;
                        rightDynamicIboxRightTopFront = Vector3.zero;
                    }
                    return;
                }

                isRightIboxValid = kinectManager.GetHandDynamicInteractionBox(primaryUserID, KinectInterop.JointType.HandRight, ref rightIboxLeftBotBack, ref rightIboxRightTopFront, ref rightDynamicIboxLeftBotBack, ref rightDynamicIboxRightTopFront, rightHandSpineMidDistance, isRightIboxValid, ref dynamicBox);
                   ...
                   kinectManager.GetJointTrackingState(primaryUserID, (int)KinectInterop.JointType.HandRight) != KinectInterop.TrackingState.NotTracked)
                {
                    ...
                    rightHandScreenPos.z = Mathf.Clamp01((rightHandPos.z - rightIboxLeftBotBack.z) / (rightIboxRightTopFront.z - rightIboxLeftBotBack.z));
                    ...
                    */
                    ...
                    {
                        leftHandPos = kinectManager.GetJointPosition(primaryUserID, (int)KinectInterop.JointType.HandLeft);
                        isRightHandInteracting = (rightHandPos.z < leftHandPos.z);
                    }
    ...
    void OnGUI()
    {
        ...
        if (debugText)
        {
            ...
            //if(isLeftHandPrimary)
            {
                ...
                sGuiText += "\nLeftDynamicIboxLeftBotBack: " + leftDynamicIboxLeftBotBack.ToString();
                sGuiText += "\nLeftDynamicIboxRightTopFront: " + leftDynamicIboxRightTopFront.ToString();
                sGuiText += "\n";
            }
            ...
            //if(isRightHandPrimary)
            {
                ...
                sGuiText += "\nRightDynamicIboxLeftBotBack: " + rightDynamicIboxLeftBotBack.ToString();
                sGuiText += "\nRightDynamicIboxRightTopFront: " + rightDynamicIboxRightTopFront.ToString();
